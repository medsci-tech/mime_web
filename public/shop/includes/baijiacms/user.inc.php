...
function member_create_new($mobile, $pwd, $userData)
{
    global $_CMS, $_W;
    if (empty($mobile) || empty($pwd)) {
        message("信息不全");
    ...
    if (!empty($hasmember['openid'])) {
        ...
        $data = array(
            'mobile' => $mobile,
            'pwd' => $pwd,
            'createtime' => strtotime($userData['created_at']),
        ...
        $data = array(
            ...
            'credit1' => $userData['credit'],
            'credit2' => $userData['credit'],
...
function member_logout()
{
    ...
    unset($_SESSION['user_login_session_key']);
    ...
    if (!empty($member['openid'])) {
        ...
        if ($type == 'usegold') {
            if ($member['credit2'] >= $fee) {
                $data = array('remark' => $remark, 'type' => $type, 'fee' => $fee, 'account_fee' => $member['credit2'] - $fee, 'createtime' => TIMESTAMP, 'openid' => $openid, 'beid' => $_CMS['beid']);
                mysqld_insert('eshop_member_paylog', $data);
                mysqld_update('eshop_member', array('credit1' => $member['credit2'] - $fee, 'credit2' => $member['credit2'] - $fee), array('openid' => $openid));
                //更新用户积分
                $md = $member['credit2'] - $fee;
                if ($member['user_type'] == 2)
                    $mysql = "update mime_doctors set credit = $md where phone=" . $member['mobile'];
                else
                    $mysql = "update mime_volunteers set credit = $md where phone=" . $member['mobile'];
                mysqld_query($mysql);
                return true;
            }
        }
